...
public class BackboneList extends LinkedList<Backbone>{
	...
	public BackboneList(final AtomList atoms, final int minNOS, final int minO, final int minBackboneLength, final int maxBackboneLength){
		...
		for(Atom atom : atoms){
			...
			candidateBackboneList.searchCandidateBackbone(candidateBackbone, minNOS, minO);
		}
	...
	// Public method (void)
	//----------------------------
	public void searchCandidateBackbone(final Backbone candidateBackbone, final int minNOS, final int minO){
		...
		for(Connection connect : tailAtom.connections){
			...
			this.searchCandidateBackbone(candidateBackbone, minNOS, minO);
	...
	public void sortForCanonicalWURCS() {
		...
		Collections.sort(this, new Comparator<Backbone>() {
			public int compare(Backbone backbone1, Backbone backbone2) {
				...
				// ４．糖をノードとした糖鎖グラフに対して計算されたEC番号が小さい糖を優先
