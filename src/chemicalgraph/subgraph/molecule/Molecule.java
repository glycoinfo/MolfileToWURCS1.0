...
public class Molecule extends ChemicalGraph{
	 ...
	 */
	public void generateWURCS(final int minNOS, final int minO, final int minBackboneLength, final int maxBackboneLength) {
		this.omitIsotope();
		this.omitCharge();
		...
		this.findBackbones(minNOS, minO, minBackboneLength, maxBackboneLength);
		this.findAglycones();
	 ...
	 * @param maxBackboneLength
	 */
	private void findBackbones(final int minNOS, final int minO, final int minBackboneLength, final int maxBackboneLength){
		// C1判定を行い短縮後の炭素鎖を取得
		BackboneList candidateBackbones = new BackboneList(this.atoms, minNOS, minO, minBackboneLength, maxBackboneLength);
